...
import { getCompanyOpsMonitor } from "./ops-monitor";

const workAgentEventWhere = { OR: [{ agentId: null }, { agent: { is: workAgentWhereClause() } }] };

export async function getDashboardSummary() {
  const [agentsRaw, projects, tasks, approvals, artifacts, criticalEvents, recentEvents, opsMonitor] = await Promise.all([
  ...
  return { agents, projects, tasks, approvals, artifacts, criticalEvents, recentEvents, failedJobs, activeTasks, highestPriorityNextAction, opsMonitor };
}
