...
export default async function AgentsPage() {
  const agents = await db.agent.findMany({ where: workAgentWhereClause(), orderBy: { updatedAt: "desc" } });
    ...
    <>
      <div className="page-head">
        <div className="titles">
          <h1>작업 에이전트</h1>
          <div className="sub">실제 작업/감시 에이전트 {agents.length}개 · 정상 {healthy}개 · 주의 필요 {degraded}개</div>
        </div>
      </div>
