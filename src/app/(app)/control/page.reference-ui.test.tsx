...
describe("ControlCenterPage reference-driven UI", () => {
  it("applies the reference board layout: command bar, observability strip, session list, and inspector", async () => {
    ...
    expect(screen.getByText("Agent Registry Table")).toBeInTheDocument();
    expect(screen.getByText("Session / Run Timeline")).toBeInTheDocument();
    expect(screen.getByText("Control 기능 책임 분리")).toBeInTheDocument();
    expect(screen.getAllByText("명령 접수 / 라우팅").length).toBeGreaterThan(0);
    expect(screen.getAllByText("실행 큐 / 작업 상태").length).toBeGreaterThan(0);
    expect(screen.getAllByText("위험 게이트 / 승인").length).toBeGreaterThan(0);
    ...
    expect(screen.queryByText("LaunchDarkly-style traces")).not.toBeInTheDocument();
    expect(screen.queryByText("OpenSea-style dense grid")).not.toBeInTheDocument();
